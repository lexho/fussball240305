...
export class Player {
    ...
    setPosition(x, y) {
        this.pos = new Position(x, y);
        let sign = (this.id % 10);
        this.feld.setPositionOnBoard(sign, x, y);
    }
    removePosition() {
    ...
    move(x, y) {
        ...
        /*console.log("x: " + x);
        console.log("y: " + y);*/
        if (this.pos.x + x < 0 || this.pos.x + x > this.feld.feld_length) {
            console.log("außerhalb des Bereichs x");
            return;
        }
        if (this.pos.y + y < 0 || this.pos.y + y > this.feld.feld_height) {
            console.log("außerhalb des Bereichs x");
            return;
        }
